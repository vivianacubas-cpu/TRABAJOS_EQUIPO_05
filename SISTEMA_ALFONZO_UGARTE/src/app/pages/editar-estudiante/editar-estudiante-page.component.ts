...
@Component({
  ...
  template: `
    <section class="page-shell reveal">
      <div class="form-card" *ngIf="modelo; else sinDatos">
        <div class="header">
          <span class="eyebrow">ACTUALIZAR DATOS</span>
          <h1>Editar estudiante</h1>
          <p>Modifica la información del estudiante y guarda los cambios.</p>
        </div>
      ...
      <ng-template #sinDatos>
        <div class="empty-state">No se encontró el estudiante.</div>
      </ng-template>
    </section>
  `,
  styles: [
...
export class EditarEstudiantePageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const estudiante = this.estudianteService.getEstudiantePorId(id);
    this.modelo = estudiante ? { ...estudiante } : null;
  }
  ...
  guardar(): void {
    ...
    if (!this.modelo.nombres.trim() || !this.modelo.apellidos.trim() || !this.modelo.codigo.trim()) {
      this.mensaje = 'Los campos principales no pueden quedar vacíos.';
      return;
    }

    this.estudianteService.actualizar(this.modelo);
